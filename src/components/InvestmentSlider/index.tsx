...
interface InvestmentSliderProps {
  ...
  symbolPosition?: 'left' | 'right'; // Optional position prop
}

const InvestmentSlider: React.FC<InvestmentSliderProps> = ({
  min, max, field, step = 1, tip, value, symbol, onChange, symbolPosition = 'left'
}) => {
    ...
    <div className="flex flex-col gap-2 w-full">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          {tip && (
            ...
            >
              ...
              {isTooltipVisible && (
                <div className="absolute z-50 p-2 text-sm text-white bg-gray-700 bg-opacity-80 rounded shadow-lg transform -translate-x-1/2 -translate-y-full"
        ...
        <div className="relative flex items-center">
          {symbolPosition === 'left' && <span className="text-lg mr-1">{symbol}</span>}
          <input
            type="number"
            value={value === 0 ? "" : value}
            ...
            max={max}
            step={step}
            style={{ paddingRight: symbolPosition === 'right' ? '24px' : undefined }}
          />
          {symbolPosition === 'right' && <span className="absolute right-2 text-lg">{symbol}</span>}
        </div>
      </div>
      <input
        type="range"
